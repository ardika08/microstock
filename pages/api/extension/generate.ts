...
async function generateOpenAIPrompt(contentType: string, platformHint: string, recentTitles: string[] = [], filename: string = ''): Promise<{systemPrompt: string; userInstruction: string}> {
  ...
  const vecteezyExtras = isVecteezy ? [
    `- title: MUST be 8-15 words (MANDATORY, not optional). Site minimum is 3 words but short titles perform poorly in search. Your title MUST contain: (1) specific color names like "cyan", "turquoise", "navy", "coral" — NOT generic "blue" or "colorful", (2) exact visual elements with quantities if applicable, (3) direction/motion words like "diagonal", "upward", "spiraling", (4) background description. REJECTED titles (too generic): "Dynamic Blue Swirls", "Abstract Colorful Background", "Flowing Waves with Gradient". REQUIRED format example: "Cyan and Turquoise Fluid Waves Flowing Diagonally Across Dark Navy Blue Background" (12 words). Count your words — if under 8, add more visual details.`,
