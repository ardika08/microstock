...
async function generateOpenAIPrompt(contentType: string, platformHint: string, recentTitles: string[] = [], filename: string = ''): Promise<{systemPrompt: string; userInstruction: string}> {
  ...
  const vecteezyExtras = isVecteezy ? [
    `- title: MINIMUM 3 words (site rule), 8-15 words STRONGLY PREFERRED for better search ranking. Must be HIGHLY SPECIFIC and CONCRETE: name the EXACT subject, specific colors (e.g. "cyan and navy" not just "blue"), textures, patterns, number of elements, direction of motion, and composition details. FORBIDDEN generic titles: "Dynamic Blue Swirls", "Abstract Colorful Background", "Beautiful Nature Scene", "Elegant Geometric Pattern" — if the title could fit thousands of different images, it is TOO GENERIC. GOOD examples: "Cyan and Turquoise Fluid Waves Flowing Diagonally on Dark Navy Background", "Five Interlocking Golden Hexagons with Glowing Edges on Black", "Soft Pink and Peach Gradient Bokeh Circles Floating Upward".`,
