...
async function callOpenAIImageToPrompt(
  ...
  const textInstruction = useJsonObject
    ? [
        ...
        '',
        'Field rules:',
        '- prompt: 100-180 words. HIGHLY DETAILED faithful reproduction with subtle variation. Describe: exact subject (matching reference closely), precise camera angle (close to reference but slightly shifted), lighting setup (matching reference mood), environment/background (same feel, minor detail changes), textures and materials (as seen in reference), color palette (matching reference), composition (similar framing with micro-adjustment). Write as one flowing dense paragraph. MUST end with a complete sentence ending in a period.',
        '- negativePrompt: 15-30 words. Elements that would make it look TOO different from reference or generic/cliché.',
        '- tags: 10-15 highly searchable microstock keywords matching the reference concept.',
        '- variants: exactly 3 alternative prompts (each 60-100 words). Each variant is STILL 80% faithful to the reference but with a DIFFERENT subtle 20% variation. Each variant MUST end with a complete sentence ending in a period.',
        '',
        'Additional rules:',
