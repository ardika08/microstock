...
export async function generateMetadata(apiKey: string, assetBrief: string) {
  const prompt = [
    ...
    "Rules: analyze the provided image itself and describe only visible content. Do not hallucinate objects, people, locations, actions, or context. The description is the primary contributor text and must be 120 to 190 characters, one sentence, no line breaks. Title must describe the dominant visible subject and composition under 180 characters. Keywords must contain 45 to 49 unique, relevant terms ordered from most important to supporting terms; avoid duplicates, filler, brand names, and irrelevant words. Do not begin every result with generic phrases such as 'A close-up', 'An abstract', or 'A beautiful' unless clearly supported by the image. Category must match the supplied category when present. Strongly prioritize the actual image over filename or generic keyword suggestions.",
